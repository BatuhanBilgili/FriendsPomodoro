...
export const PhysicsWorld = ({
	...
	useEffect(() => {
		...
		// Create user balls (smaller)
		const otherUsers = users.filter(u => u.userId !== currentUser?.userId).slice(0, 4);
		otherUsers.forEach((user, index) => {
			const radius = 18;
			const x = 50 + index * 45;
			const y = 80;

			const userBall = Bodies.circle(x, y, radius, {
				restitution: 0.8,
				friction: 0.08,
				frictionAir: 0.015,
				density: 0.0005,
			...
			Body.setVelocity(userBall, {
				x: (Math.random() - 0.5) * 4,
				y: (Math.random() - 0.5) * 3
			});

			Composite.add(engine.world, userBall);
		});
